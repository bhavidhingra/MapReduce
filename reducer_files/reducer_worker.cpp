...
void ReducerTracker::index_all_words(int job_id)
{
...
void ReducerTracker::mapper_request_handler(int mapper_sock, string req_str)
{
    ...
    switch(problem)
    {
        ...
        case Problem::INVERTED_INDEX:
        {
            ...
            if(num_job_files(job_id) == num_mappers_alive)
            {
                thread counting_thread(&ReducerTracker::index_all_words, this, job_id);
                counting_thread.detach();
            }
            break;
        }
...
int main(int argc, char* argv[])
{
    if(argc != 3)
    {
        //status_print(FAILURE, "Reducer Launch command : \"/reducer <REDUCER_IP>:<PORT> <reducer_log_file>\"");
        //cout << endl;
        return 0;
    }
